...
function getRandomRecipe(recipes) {
    ...
    return recipes[randomIndex]; // Return just one recipe object
  }
  ...
  function renderRecipes(recipes) {
      ...
      // If the recipes array has more than one recipe, we iterate and display all of them
      if (Array.isArray(recipes)) {
          let html = recipes.map(recipe => recipeTemplate(recipe)).join("");
          recipeContainer.innerHTML = html;
      } else {
          // If only one recipe is passed, render that one
          recipeContainer.innerHTML = recipeTemplate(recipes);
      }
  }
  
  function init() {
    const randomRecipe = getRandomRecipe(recipes); // Get a random recipe from the recipes array
    renderRecipes(randomRecipe); // Render the random recipe (not an array)
  }
  
  document.addEventListener("DOMContentLoaded", function() {
      init(); // Call init when the document is loaded
  });
  ...
  function searchHandler(e) {
      ...
      // Render the filtered recipes (passed as an array)
  ...
  function filterRecipes(query) {
      const filtered = recipes.filter(recipe => {
          ...
          return recipe.name.toLowerCase().includes(lowerCase) ||
          recipe.description.toLowerCase().includes(lowerCase) ||
          recipe.tags.some(tag => tag.toLowerCase().includes(lowerCase)) ||
          recipe.recipeIngredient.some(ingredient => ingredient.toLowerCase().includes(lowerCase));
      });
...
//     let filtered = recipes.filter(recipe => recipe.name.toLowerCase().includes(searchText) || recipe.description.toLowerCase().includes(searchText) || recipe.tags.some(tag => tag.toLowerCase().includes(searchText)) || recipe.recipeIngredient.some(ingredient => ingredient.toLowerCase().includes(searchText))
//     );
